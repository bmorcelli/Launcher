...
const ESP_CHIP_FAMILY: Record<string, string> = {
  "32": "ESP32",
  s3: "ESP32-S3",
  c3: "ESP32-C3",
  s2: "ESP32-S2",
...
const ESP_ENV_NAME: Record<string, string> = {
  "32": "esp32",
  s3: "esp32s3",
  s2: "esp32s2",
  c3: "esp32c3",
...
type FlashPart = { url: string; address: number };

const CATALOG_FLASH_STYLE_ID = "catalog-flash-dialog-style";

const ensureCatalogFlashStyles = () => {
  if (document.getElementById(CATALOG_FLASH_STYLE_ID)) return;
...
type CatalogFlashPhase = "idle" | "connecting" | "preparing" | "erasing" | "writing" | "done" | "error";
interface CatalogFlashState { phase: CatalogFlashPhase; message: string; progress: number | null; }

const catalogSleep = (ms: number) => new Promise<void>((r) => setTimeout(r, ms));

const flashFirmwareParts = async (
  port: SerialPort,
  parts: FlashPart[],
  ...
  const fire = (phase: CatalogFlashPhase, message: string, progress: number | null = null) =>
  ...
  const bytesPerPart: number[] = fileArray.map((f) => f.data.byteLength);
...
const openCatalogFlashDialog = (parts: FlashPart[], expectedChipFamily: string, firmwareName: string, isApp = false) => {
  ...
  const close = () => { if (!isFlashing) backdrop.remove(); };
  ...
  const updateUI = (s: CatalogFlashState) => {
  ...
  installBtn.addEventListener("click", () => {
    if (installBtn.dataset.wfDone === "true") { backdrop.remove(); return; }
    if (!isFlashing) void startFlash();
  });
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const buildCard = (entry: FirmwareEntry) => {
    ...
    appFlashButton.className = "button";
    appFlashButton.style.display = "none";
    appFlashButton.style.background = "linear-gradient(135deg, rgba(255,140,0,0.92) 0%, rgba(255,80,0,0.88) 100%)";
    appFlashButton.style.color = "#1a0800";
    appFlashButton.style.fontWeight = "600";
    appFlashButton.style.borderRadius = "var(--radius-lg, 24px)";
    appFlashButton.style.border = "none";
    appFlashButton.style.padding = "10px 18px";
    appFlashButton.style.cursor = "pointer";
