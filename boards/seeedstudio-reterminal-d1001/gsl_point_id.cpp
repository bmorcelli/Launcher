...
static int Sqrt(int d) {
    ...
    for (i = 14; i >= 0; i--) {
        if ((ret + (0x1 << i)) * (ret + (0x1 << i)) <= d) ret |= (0x1 << i);
    }
    return ret;
}
...
static UINT PointRange(int x0, int y0, int x1, int y1) {
    if (x0 < 1) /* && x1>=1 */ {
        if (x0 != x1) y0 = y1 + (y0 - y1) * (1 - x1) / (x0 - x1);
        x0 = 1;
    }
    if (x0 >= (int)drv_num_nokey * 64) {
        if (x0 != x1) y0 = y1 + (y0 - y1) * ((int)drv_num_nokey * 64 - x1) / (x0 - x1);
        x0 = drv_num_nokey * 64 - 1;
    }
    if (y0 < 1) {
        if (y0 != y1) x0 = x1 + (x0 - x1) * (1 - y1) / (y0 - y1);
        y0 = 1;
    }
    if (y0 >= (int)sen_num_nokey * 64) {
        if (y0 != y1) x0 = x1 + (x0 - x1) * ((int)sen_num_nokey * 64 - y1) / (y0 - y1);
        y0 = sen_num_nokey * 64 - 1;
    }
    if (x0 < 1) x0 = 1;
    if (x0 >= (int)drv_num_nokey * 64) x0 = drv_num_nokey * 64 - 1;
    if (y0 < 1) y0 = 1;
    if (y0 >= (int)sen_num_nokey * 64) y0 = sen_num_nokey * 64 - 1;
    return (x0 << 16) + y0;
}
...
static void PointCoor(void) {
    ...
    for (i = 0; i < point_num; i++) {
        if (global_state.other.ex) point_now[i].all &= (FLAG_COOR_EX | FLAG_KEY | FLAG_ABLE);
        else point_now[i].all &= (FLAG_COOR | FLAG_KEY | FLAG_ABLE);
    }
}
...
static void PointRepeat(void) {
    ...
    if (point_near) point_near--;
    if (prev_num > point_num) point_near = 8;
    if (point_repeat[0] == 0 || point_repeat[1] == 0) {
        if (point_near) pn = 96;
        else pn = 32;
    } else {
        if (point_near) pn = point_repeat[1];
        else pn = point_repeat[0];
    }
    for (i = 0; i < POINT_MAX; i++) {
        if (point_now[i].all == 0) continue;
        if (point_now[i].other.key) continue;
        ...
        for (j = i + 1; j < POINT_MAX; j++) {
            if (point_now[j].all == 0) continue;
            if (point_now[j].other.key) continue;
...
static void PointPointer(void) {
    ...
    point_n++;
    if (point_n >= PP_DEEP * PS_DEEP * PR_DEEP * PRESSURE_DEEP) point_n = 0;
    pn = point_n % PP_DEEP;
    for (i = 0; i < PP_DEEP; i++) {
        pp[i] = point_array[pn];
        if (pn == 0) pn = PP_DEEP - 1;
        else pn--;
    }
    pn = point_n % PS_DEEP;
    for (i = 0; i < PS_DEEP; i++) {
        ps[i] = point_array[pn + PP_DEEP];
        if (pn == 0) pn = PS_DEEP - 1;
        else pn--;
    }
    pn = point_n % PR_DEEP;
    for (i = 0; i < PR_DEEP; i++) {
        pr[i] = point_array[pn + PP_DEEP + PS_DEEP];
        if (pn == 0) pn = PR_DEEP - 1;
        else pn--;
    }
    pn = point_n % PRESSURE_DEEP;
    for (i = 0; i < PRESSURE_DEEP; i++) {
        pa[i] = pressure_array[pn];
        if (pn == 0) pn = PRESSURE_DEEP - 1;
        else pn--;
    }
...
static unsigned int CC128(unsigned int x, unsigned int **coe, int k) {
    if (k == 3) {
        ...
        else return (x & ~127) + (coe[((x >> 6) & 1) ^ 1][x & 63] & 127);
    }
    return 0;
}
static unsigned int CCO(unsigned int x, unsigned int coe[], int k) {
    if (k == 0) {
        if (x & 32) return (x & ~31) + (31 - (coe[31 - (x & 31)] & 31));
        else return (x & ~31) + (coe[x & 31] & 31);
    }
    if (k == 1) {
        if (x & 64) return (x & ~63) + (63 - (coe[63 - (x & 63)] & 63));
        else return (x & ~63) + (coe[x & 63] & 63);
    }
    if (k == 2) return (x & ~63) + (coe[x & 63] & 63);
...
#define LINE_SIZE 4
    ...
    if ((coordinate_correct_able & 0xf) == 0) return;
    ...
    if (kx == 3 || ky == 3 || kx == 4 || ky == 4) {
        i = 0;
        if (kx == 3 || kx == 4) px[1] = multi_group[i++];
        if (ky == 3 || ky == 4) py[1] = multi_group[i++];
    } else {
        for (i = 0; i < LINE_SIZE; i++) {
        ...
        j = 1;
        for (i = 0; i < LINE_SIZE; i++)
            if (multi_x[i].range && multi_x[i].group < LINE_SIZE) px[j++] = multi_group[multi_x[i].group];
        j = 1;
        for (i = 0; i < LINE_SIZE; i++)
            if (multi_y[i].range && multi_y[i].group < LINE_SIZE) py[j++] = multi_group[multi_y[i].group];
    }
    for (i = 0; i < (int)point_num && i < POINT_MAX; i++) {
        if (point_now[i].all == 0) break;
        if (point_now[i].other.key != 0) continue;
        if (point_now[i].other.x >= edge_size && point_now[i].other.x <= drv_num_nokey * 64 - edge_size) {
            if (global_state.other.active) {
                ...
                if (point_now[i].other.x & 64) point_now[i].other.x = CCO(point_now[i].other.x, px[0], 2);
                else point_now[i].other.x = CCO(point_now[i].other.x, px[1], 2);
            } else {
        ...
        if (point_now[i].other.y >= edge_size && point_now[i].other.y <= sen_num_nokey * 64 - edge_size) {
            if (global_state.other.active) {
                ...
                if (point_now[i].other.y & 64) point_now[i].other.y = CCO(point_now[i].other.y, py[0], 2);
                else point_now[i].other.y = CCO(point_now[i].other.y, py[1], 2);
            } else {
                for (j = 0; j < LINE_SIZE + 1; j++) {
...
static void PointPredict(void) {
    ...
    for (i = 0; i < POINT_MAX; i++) {
        if (pp[1][i].all != 0) {
            if (global_state.other.interpolation != 0 && global_state.other.interpolation != INTE_INIT &&
                pp[3][i].all && pp[3][i].other.fill == 0) {
                if (pp[4][i].all && pp[5][i].all && pp[5][i].other.fill == 0) PointPredictD3(i);
                else PointPredictD2(i);
            } else if (
                global_state.other.interpolation || pp[2][i].all == 0 || pp[2][i].other.fill != 0 ||
                pp[3][i].other.fill != 0 || pp[1][i].other.key != 0 || global_state.other.only
            ) {
                PointPredictOne(i);
            } else if (
                pp[2][i].all != 0 && (avg[0] != avg[1] || avg[1] != avg[2]) && avg[0] != 0 && avg[1] != 0
            ) {
                PointPredictSpeed(i);
            } else if (pp[2][i].all != 0) {
                if (pp[3][i].all != 0) PointPredictThree(i);
                else PointPredictTwo(i);
            }
            pp[0][i].all |= FLAG_FILL;
            pa[0][i] = pa[1][i];
        } else pp[0][i].all = 0x0fff0fff;
        if (pp[1][i].other.key) pp[0][i].all |= FLAG_KEY;
    }
}
...
static void DistanceInit(struct gsl_DISTANCE_TYPE *p) {
    ...
    for (i = 0; i < POINT_MAX * POINT_MAX; i++) *p_int++ = 0x7fffffff;
}
...
static void DistanceIgnore(struct gsl_DISTANCE_TYPE *p) {
    ...
    for (i = 0; i < POINT_MAX; i++) p->d[p->j][i] = 0x7fffffff;
    for (j = 0; j < POINT_MAX; j++) p->d[j][p->i] = 0x7fffffff;
}
...
static void PointId(void) {
    ...
    for (i = 0; i < POINT_MAX; i++) {
        if (pp[0][i].other.predict == 0 || pp[1][i].other.fill != 0) id_speed[i] = id_first_coe;
        else {
            id_speed[i] = SpeedGet(PointDistance(&pp[1][i], &pp[0][i]));
            j = SpeedGet(PointDistance(&pp[2][i], &pp[1][i]));
            if (id_speed[i] < (unsigned int)j) id_speed[i] = j;
        }
    }
    for (i = 0; i < POINT_MAX; i++) {
        if (pp[0][i].all == FLAG_COOR) continue;
    ...
    if (global_state.other.only || global_state.other.active) {
        do {
            ...
            } else pp[0][0].all = point_now[0].all;
            for (i = 0; i < POINT_MAX; i++) point_now[i].all = 0;
        } while (0);
        point_num = 1;
    } else {
        for (j = 0; j < point_num && j < POINT_MAX; j++) {
            if (DistanceMin(&distance) == 0) break;
...
static void PointNewId(void) {
    ...
    for (j = 0; j < POINT_MAX; j++)
        if ((pp[0][j].all & FLAG_COOR) == FLAG_COOR) pp[0][j].all = 0;
    for (j = 0; j < POINT_MAX; j++) {
        if (point_now[j].all != 0) {
            if (point_now[j].other.able) continue;
...
static void PointOrder(void) {
    ...
    for (i = 0; i < POINT_MAX; i++) {
        if (pp[0][i].other.fill == 0) continue;
...
static void PointCross(void) {
    ...
    for (j = 0; j < POINT_MAX; j++) {
        for (i = j + 1; i < POINT_MAX; i++) {
            if (pp[0][i].all == 0 || pp[0][j].all == 0 || pp[1][i].all == 0 || pp[1][j].all == 0) continue;
...
static unsigned int PointDelayAvg(int i) {
    ...
    if (id_flag.other.first_avg == 0) return TRUE;
    if (pp[0][i].all) {
    ...
    if (pp[1][i].all == 0) return FALSE;
    for (j = 1; j <= point_delay[i].other.delay; j++)
        if (pp[j][i].all == 0) break;
    len = j - 1;
    if (len < 1 + (point_delay[i].other.delay - point_delay[i].other.report)) return FALSE;
...
static void PointDelay(void) {
    ...
    for (i = 0; i < POINT_MAX; i++) {
        if (report_delay == 0 && delay_key == 0) {
            point_delay[i].all = 0;
            if (pp[0][i].all) point_delay[i].other.able = 1;
            if (pr[0][i].all == 0) point_delay[i].other.mask = 0;
            continue;
        }
        if (pp[0][i].all != 0 && point_delay[i].other.init == 0 && point_delay[i].other.able == 0) {
            ...
            if (delay_key && pp[0][i].other.key) {
                ...
                point_delay[i].other.dele =
                    (report_delete >> 3 * ((point_num > 10 ? 10 : point_num) - 1)) & 0x7;
        ...
        if (point_delay[i].other.able == 0 && point_delay[i].other.init != 0) {
            for (j = 0; j <= (int)point_delay[i].other.delay; j++) {
                if (pp[j][i].all == 0 || pp[j][i].other.fill != 0 || pp[j][i].other.able != 0) break;
            }
            if (j <= (int)point_delay[i].other.delay) continue;
            if (PointDelayAvg(i)) point_delay[i].other.able = 1;
            else j = 0;
            if (id_flag.other.first_avg) point_delay[i].other.report = point_delay[i].other.dele;
        }
        ...
        if (point_delay[i].other.able == 0) continue;
        if (report_delete == 0 && point_delay[i].other.report) {
            if (PointDistance(&pp[point_delay[i].other.report][i], &pp[point_delay[i].other.report - 1][i]) <
                3 * 3) {
                point_delay[i].other.report--;
                if (point_delay[i].other.dele) point_delay[i].other.dele--;
            }
        }
    }
}
...
static unsigned int PointMOne(unsigned int x0, unsigned int x1) {
    ...
    if (e2 == 0) e2 = 24;
    if (x1 >= x0) return 0;
    if (x1 < (edge_start & 0xff) && x1 * e1 / 16 < x0) return 1;
    else if (x1 < (edge_start & 0xff) * 2 && x1 * e2 / 16 < x0) return 1;
    return 0;
}
...
static void PointMenu(void) {
    ...
    if (PointDistance(&pp[0][0], &pp[1][0]) >= edge_dis) {
        if (PointMOne(pp[0][0].other.x, pp[1][0].other.x)) pr[1][0].other.x = 1;
        if (PointMOne(pp[0][0].other.y, pp[1][0].other.y)) pr[1][0].other.y = 1;
...
static int MedianSpeedOver(int id, int deep) {
    ...
    deep = deep / 2 - 1;
    if (deep < 0 || deep > 3) return TRUE;
    dis = median_dis[deep] * median_dis[deep];
    for (i = 0; i <= deep && i < POINT_DEEP; i++) {
        if (PointDistance(&ps[i][id], &ps[i + 1][id]) > dis) speed_over++;
    }
    if (speed_over >= 2) return TRUE;
    return FALSE;
}
...
static void PointMedian(void) {
    ...
    for (i = 0; i < POINT_MAX; i++) {
        if (filter_deep[i] < 3) deep = 3;
        else deep = filter_deep[i] + 2;
        if (deep >= PS_DEEP) deep = PS_DEEP - 1;
        deep |= 1;
        for (; deep >= 3; deep -= 2) {
            if (MedianSpeedOver(i, deep)) continue;
            for (j = 0; j < deep; j++) {
...
static void PointFilter(void) {
    ...
    for (i = 0; i < POINT_MAX; i++) pr[0][i].all = ps[0][i].all;

    for (i = 0; i < POINT_MAX; i++) {
        if (pr[0][i].all != 0 && pr[1][i].all == 0) {
            for (j = 1; j < PR_DEEP; j++) pr[j][i].all = ps[0][i].all;
            for (j = 1; j < PS_DEEP; j++) ps[j][i].all = ps[0][i].all;
        }
    }
    if (filter_able >= 0 && filter_able <= 1) return;
    if (filter_able > 1) {
        for (i = 0; i < 8; i++) {
            ps_c[i] = (filter_coe[i / 4] >> ((i % 4) * 8)) & 0xff;
            pr_c[i] = (filter_coe[i / 4 + 2] >> ((i % 4) * 8)) & 0xff;
            if (ps_c[i] >= 0x80) ps_c[i] |= 0xffffff00;
            if (pr_c[i] >= 0x80) pr_c[i] |= 0xffffff00;
        }
        for (i = 0; i < POINT_MAX; i++) FilterOne(i, ps_c, pr_c, filter_able);

    } else if (filter_able == -1) {
        PointMedian();
    } else if (filter_able < 0) {
        for (i = 0; i < 4; i++) filter_speed[i + 1] = median_dis[i];
        ...
        for (i = 0; i < POINT_MAX; i++) {
            ...
            if (filter_deep[i] > 0 && speed_now > filter_speed[filter_deep[i] + 1 - 2]) filter_deep[i]--;
            else if (filter_deep[i] < 3 && speed_now < filter_speed[filter_deep[i] + 1 + 2]) filter_deep[i]++;
    ...
    if (p->other.key == FALSE) {
        if (id_flag.other.ignore_pri == 0) {
            if (ignore_y[0] != 0 || ignore_y[1] != 0) {
                if (y < ignore_y[0]) return 0;
                if (ignore_y[1] <= screen_y_max / 2 && y > screen_y_max - ignore_y[1]) return 0;
                if (ignore_y[1] >= screen_y_max / 2 && y > ignore_y[1]) return 0;
            }
            if (ignore_x[0] != 0 || ignore_x[1] != 0) {
                if (x < ignore_x[0]) return 0;
                if (ignore_x[1] <= screen_x_max / 2 && x > screen_x_max - ignore_x[1]) return 0;
                if (ignore_x[1] >= screen_x_max / 2 && x > ignore_x[1]) return 0;
            }
        }
        if (y <= (int)edge_cut[2]) y = (int)edge_cut[2] + 1;
        if (y >= screen_y_max - (int)edge_cut[3]) y = screen_y_max - (int)edge_cut[3] - 1;
        if (x <= (int)edge_cut[0]) x = (int)edge_cut[0] + 1;
        if (x >= screen_x_max - (int)edge_cut[1]) x = screen_x_max - (int)edge_cut[1] - 1;
        if (global_flag.other.opposite_x) y = screen_y_max - y;
        if (global_flag.other.opposite_y) x = screen_x_max - x;
        ...
        if (y < 0) y = 0;
        if (x < 0) x = 0;
        if ((key_map_able & 0x1) != FALSE && KeyMap(&x, &y) == 0) return 0;
    }
...
static void PointReport(struct gsl_touch_info *cinfo) {
    ...
    for (i = 0; i < POINT_MAX; i++) data[i] = dp[i] = 0;
    num = 0;
    if (global_flag.other.id_over) {
        for (i = 0; i < POINT_MAX && num < point_num_max; i++) {
            if (point_delay[i].other.mask || point_delay[i].other.able == 0) continue;
            if (point_delay[i].other.report >= PR_DEEP - 1) continue;
            if (pr[point_delay[i].other.report + 1][i].other.able == 0) continue;
        ...
        for (i = 0; i < POINT_MAX && num < point_num_max; i++) {
            if (point_delay[i].other.mask || point_delay[i].other.able == 0) continue;
            if (point_delay[i].other.report >= PR_DEEP) continue;
            if (pr[point_delay[i].other.report][i].all == 0) continue;
    ...
    if (id_flag.other.id_prec_able == FALSE) return;
    if (prec_id.all == 0 && point_num == 1) {
        if ((point_now[0].all >> 28) > 1) prec_id.other.id = (point_now[0].all >> 28);
        else prec_id.other.id = 0xff;
    }
    ...
    if (point_num == 0) prec_id.all = 0;
    else prec_id.other.num = (unsigned char)point_num;
}
...
static void PointRound(void) {
    ...
    if (id_flag.other.round == 0 || id_flag.other.stretch_off) return;
    if (screen_x_max == 0 || screen_y_max == 0) return;
    id = 0;
    for (i = 0; i < 4 * 4 * 2; i++) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (stretch->down[i].range)
            stretch->down[i].range =
                stretch->down[i].range * sen_num_nokey * drv_num_nokey * 64 / screen_x_max;
        if (stretch->left[i].range)
            stretch->left[i].range =
                stretch->left[i].range * sen_num_nokey * drv_num_nokey * 64 / screen_y_max;
        if (stretch->right[i].range)
            stretch->right[i].range =
                stretch->right[i].range * sen_num_nokey * drv_num_nokey * 64 / screen_y_max;
    }

    x0 = 64 * sen_num_nokey * drv_num_nokey / 2;
    y0 = x0;
    for (id = 0; id < POINT_MAX; id++) {
        if (point_now[id].all == 0 || point_now[id].other.key != 0) continue;
...
static void PointEdge(void) {
    ...
    if (id_flag.other.round || id_flag.other.stretch_off) return;
    if (screen_x_max == 0 || screen_y_max == 0) return;
    id = 0;
    for (i = 0; i < 4 * 4 * 2; i++) {
        if (global_state.other.active) sac[i] = stretch_active[i];
        else sac[i] = stretch_array[i];
        if (sac[i]) id++;
    }
    if (id == 0) return;
    stretch = (struct STRETCH_TYPE_ALL *)sac;
    for (i = 0; i < 4; i++) {
        if (id_flag.other.screen_core) break;
        if (stretch->right[i].range > screen_y_max * 64 / 128 ||
            stretch->down[i].range > screen_x_max * 64 / 128 || id_flag.other.screen_real) {
            for (i = 0; i < 4; i++) {
                if (stretch->up[i].range)
    ...
    for (id = 0; id < POINT_MAX; id++) {
        ...
        data[1] = y;
        for (i = 0; i < 4; i++) {
            if (stretch->left[i].range == 0) break;
        ...
        y = data[1] - data[0];
        if (y <= 0) y = 1;
        if (y >= (int)sen_num_nokey * 64) y = sen_num_nokey * 64 - 1;

        data[0] = 0;
        data[1] = sen_num_nokey * 64 - y;
        for (i = 0; i < 4; i++) {
            if (stretch->right[i].range == 0) break;
        ...
        x = data[1] - data[0];
        if (x <= 0) x = 1;
        if (x >= (int)drv_num_nokey * 64) x = drv_num_nokey * 64 - 1;

        data[0] = 0;
        data[1] = drv_num_nokey * 64 - x;
        for (i = 0; i < 4; i++) {
            if (stretch->down[i].range == 0) break;
...
static void PointStretch_for(int *dc_p, int *ds_p) {
    ...
    for (i = 0; i < POINT_MAX; i++) {
        ...
        if (id_flag.other.first_avg && point_delay[i].other.able == 0) continue;
        if ((point_shake & (0x1 << i)) == 0) continue;
        if (dc[len] == 3) /* dc == 2 */ {
            ...
            if (dn < save_dn[i]) {
                ...
                save_dn[i] = dn;
                ps[0][i].other.x =
                    (int)ps[1][i].other.x + (((int)pp[0][i].other.x - (int)ps[1][i].other.x) * dr) / 128;
                ps[0][i].other.y =
                    (int)ps[1][i].other.y + (((int)pp[0][i].other.y - (int)ps[1][i].other.y) * dr) / 128;
                continue;
            }
            for (j = 0; j <= len; j++) {
                if (j == len || dn == 0) {
...
static void PointStretch(void) {
    ...
    for (i = 0; i < POINT_MAX; i++) ps[0][i].all = pp[0][i].all;

    for (i = 0; i < POINT_MAX; i++) {
        if (pp[0][i].all == 0 || pp[0][i].other.key) {
            point_shake &= ~(0x1 << i);
            if (i == 0) point_edge.rate = 0;
            continue;
        }
        if (i == 0) {
            if (edge_first != 0 && ps[1][i].all == 0) {
                ...
                if (point_edge.coor.all != ps[0][i].all) {
                    ...
                    if (point_edge.dis) point_edge.rate = 0x1000;
                }
            }
            if (point_edge.rate != 0 && point_edge.dis != 0) {
                ...
                if (temp >= point_edge.dis * edge_first_coe / 0x80) {
                    ...
                    if (temp < point_edge.rate) point_edge.rate = temp;
                }
                ps[0][i].other.x = point_edge.coor.other.x + (ps[0][i].other.x - point_edge.coor.other.x) *
                                                                 (0x1000 - point_edge.rate) / 0x1000;
                ps[0][i].other.y = point_edge.coor.other.y + (ps[0][i].other.y - point_edge.coor.other.y) *
                                                                 (0x1000 - point_edge.rate) / 0x1000;
            }
        }
        if (ps[1][i].all == 0) {
            ...
            id_flag.other.first_avg && (point_shake & (0x1 << i)) == 0 && pp[0][i].all &&
            ...
            if (PointDistance(&ps[0][i], &ps[1][i]) < (unsigned int)shake_min) {
                if (point_delay[i].other.able) ps[0][i].all = ps[1][i].all;
                else {
                    for (j = 1; j < PS_DEEP; j++) ps[j][i].all = ps[0][i].all;
                    for (j = 0; j < PR_DEEP; j++) pr[j][i].all = ps[0][i].all;
                }
                continue;
            } else point_shake |= (0x1 << i);
        }
    }
    for (i = 0; i < len; i++) {
    ...
    if (len == 1) {
        ...
        for (i = 0; i < POINT_MAX; i++) {
            ...
            if (dn == 0 || dr == 0) {
                ...
                ps[0][i].other.x =
                    (int)ps[1][i].other.x + ((int)pp[0][i].other.x - (int)ps[1][i].other.x) * dr / dn;
                ps[0][i].other.y =
                    (int)ps[1][i].other.y + ((int)pp[0][i].other.y - (int)ps[1][i].other.y) * dr / dn;
            }
            if (dc[0] > 0) {
        ...
        if (shake_all[0].coe >= 128 || shake_all[0].coe <= shake_all[1].coe) {
            ...
            ds[0] =
                ds[1] + (128 - shake_all[0].coe) * (ds[1] - ds[2]) / (shake_all[0].coe - shake_all[1].coe);
            dc[0] = 128;
        }
...
static void ResetMask(void) {
    if (reset_mask_send) reset_mask_send = 0;

    if (global_state.other.mask) return;
    if (reset_mask_dis == 0 || reset_mask_type == 0) return;
    if (reset_mask_max == 0xfffffff1) {
        if (point_num == 0) reset_mask_max = 0xf0000000 + 1;
        return;
    }
    ...
    reset_mask_count++;
    if (reset_mask_max == 0) reset_mask_max = pp[0][0].all;
    else if (
        PointDistance((union gsl_POINT_TYPE *)(&reset_mask_max), pp[0]) >
...
static int ConfigCoorMulti(unsigned int data[]) {
    ...
    for (i = 0; i < 4; i++) {
        if (data[247 + i] != 0) {
            if ((data[247 + i] & 63) == 0 && (data[247 + i] >> 16) < 4) n++;
            else return FALSE;
        }
        if (data[251 + i] != 0) {
            if ((data[251 + i] & 63) == 0 && (data[251 + i] >> 16) < 4) n++;
            else return FALSE;
        }
    }
    ...
    for (j = 0; j < n; j++) {
        for (i = 0; i < 64; i++) {
            if (data[256 + j * 64 + i] >= 64) return FALSE;
            if (i) {
                if (data[256 + j * 64 + i] < data[256 + j * 64 + i - 1]) return FALSE;
            }
        }
    }
    return TRUE;
...
static int ConfigFilter(unsigned int data[]) {
    ...
    if (data[242] > 1 && (data[255] <= 256)) {
        ...
        if (sum == data[242] || sum + data[242] == 0) return TRUE;
    }
    return FALSE;
}
...
static int ConfigKeyMap(unsigned int data[]) {
    ...
    if (data[217] != 1) return FALSE;
    for (i = 0; i < 8; i++) {
        if (data[218 + 2] == 0) return FALSE;
        if ((data[218 + i * 3 + 0] >> 16) > (data[218 + i * 3 + 0] & 0xffff)) return FALSE;
        if ((data[218 + i * 3 + 1] >> 16) > (data[218 + i * 3 + 1] & 0xffff)) return FALSE;
    }
    return TRUE;
}
...
static void DiagonalCompress(union gsl_POINT_TYPE *p, int type, int dis, int dis_max) {
    ...
    if (dis_max == 0) return;
    if (dis > dis_max) cp_ceof = (dis - dis_max) * 128 / (3 * dis_max) + 128;
    else cp_ceof = 128;
    if (cp_ceof > 256) cp_ceof = 256;
    x = p->other.x;
    y = p->other.y;
    if (type) y = (int)sen_num_nokey * 64 - y;
    x *= (int)sen_num_nokey;
    y *= (int)drv_num_nokey;
    tx = x;
    ty = y;
    ...
    x /= (int)sen_num_nokey;
    y /= (int)drv_num_nokey;
    if (type) y = sen_num_nokey * 64 - y;
    if (x < 1) x = 1;
    if (y < 1) y = 1;
    if (x >= (int)drv_num_nokey * 64) x = drv_num_nokey * 64 - 1;
    if (y >= (int)sen_num_nokey * 64) y = (int)sen_num_nokey * 64 - 1;
    p->other.x = x;
    p->other.y = y;
}
    ...
    for (i = 0; i < POINT_MAX; i++) {
        if (ps[0][i].all == 0 || ps[0][i].other.key != 0) {
            ...
            if ((ps[0][i].other.x <= diagonal_start && ps[0][i].other.y <= diagonal_start) ||
                ...
                (ps[0][i].other.x <= diagonal_start &&
                 ps[0][i].other.y >= sen_num_nokey * 64 - diagonal_start) ||
                (ps[0][i].other.x >= drv_num_nokey * 64 - diagonal_start &&
                 ps[0][i].other.y <= diagonal_start)
            )
                point_corner |= 0x3 << i * 2;
            else point_corner |= 0x1 << i * 2;
        }
...
static int PointSlope(int i, int j) {
    ...
    y = y * y;
    if (x + y == 0) return -1;
    if (x > y) return x * 1024 / (x + y);
    else return y * 1024 / (x + y);
}
...
static void PointExtend(void) {
    ...
    if (point_extend == 0) return;
    for (i = 0; i < POINT_MAX; i++) {
        if (pr[0][i].other.fill == 0) continue;
        for (j = 0; j < extend_len; j++) {
            if (pr[j][i].all == 0) break;
        }
        if (j < extend_len) continue;
        if (PointDistance(&pr[1][i], &pr[2][i]) < 16 * 16) continue;
        t = PointSlope(i, 1);
        for (j = 2; j < extend_len - 1; j++) {
            t2 = PointSlope(i, j);
            if (t2 < 0 || t2 < t * (128 - point_extend) / 128 || t2 > t * (128 + point_extend) / 128) break;
        }
        if (j < extend_len - 1) continue;
...
static void PointPressure(void) {
    ...
    for (i = 0; i < POINT_MAX; i++) {
        if (pa[0][i] != 0 && pa[1][i] == 0) {
            ...
            for (j = 1; j < PRESSURE_DEEP; j++) pa[j][i] = pa[0][i];
            continue;
        }
...
static void PressMask(void) {
    ...
    for (i = 0; i < POINT_MAX; i++) {
        ...
        if (point_delay[i].other.delay >= 1 && point_delay[i].other.pres == 0) {
            if (pa[0][i] > pa[1][i]) point_delay[i].other.able = 0;
            else point_delay[i].other.pres = 1;
        }
    }
    for (i = 0; i < POINT_MAX; i++) {
        if (pr[0][i].all == 0) continue;
        if (point_delay[i].other.mask == 0 && pressure_report[i] < press_max + 7) continue;
        point_delay[i].other.able = 0;
        ...
        if (press_range == 0) continue;
        for (j = 0; j < POINT_MAX; j++) {
            if (i == j) continue;
            if (pr[0][j].all == 0 || point_delay[j].other.able == 0) continue;
...
static void PressMove(void) {
    ...
    if (press_move == 0) return;
    if (pr[0][0].all == 0) goto press_move_err;
    for (i = 1; i < POINT_MAX; i++) {
        if (pr[0][i].all) goto press_move_err;
    }
    if (pressure_report[0] < (press_move & 0xff) + 7) goto press_move_err;
...
int gsl_PressMove(void) {
    if (point_press_move.all <= 4) return point_press_move.all;
    else return 0;
}
...
void gsl_ReportPressure(unsigned int *p) {
    ...
    for (i = 0; i < POINT_MAX; i++) {
        if (i < point_num) {
            if (pressure_now[i] == 0) p[i] = 0;
            else if (pressure_now[i] <= 7) p[i] = 1;
            else if (pressure_now[i] > 63 + 7) p[i] = 63;
            else p[i] = pressure_now[i] - 7;
        } else p[i] = 0;
    }
}
...
static void gsl_id_reg_init(int flag) {
    ...
    for (j = 0; j < POINT_DEEP; j++)
        for (i = 0; i < POINT_MAX; i++) point_array[j][i].all = 0;
    for (j = 0; j < PRESSURE_DEEP; j++)
        for (i = 0; i < POINT_MAX; i++) pressure_array[j][i] = 0;
    for (i = 0; i < POINT_MAX; i++) {
        point_delay[i].all = 0;
        filter_deep[i] = 0;
        point_decimal[i].all = 0;
    }
    for (i = 0; i < AVG_DEEP; i++) avg[i] = 0;
    point_edge.rate = 0;
    point_n = 0;
    if (flag) point_num = 0;
    prev_num = 0;
...
static int DataCheck(void) {
    if (drv_num == 0 || drv_num_nokey == 0 || sen_num == 0 || sen_num_nokey == 0) return 0;
...
void gsl_DataInit(unsigned int *conf_in) {
    ...
    drv_num_nokey = 16;
    sen_num_nokey = 10;
    for (i = 0; i < 4; i++) edge_cut[i] = 0;
    for (i = 0; i < 32; i++) stretch_array[i] = 0;
    for (i = 0; i < 16; i++) shake_all_array[i] = 0;
    reset_mask_dis = 0;
    reset_mask_type = 0;
    edge_start = 0;
    diagonal = 0;
    point_extend = 0;
    key_map_able = 0;
    for (i = 0; i < 8 * 3; i++) key_range_array[i] = 0;
    filter_able = 0;
    ...
    stretch_mult = 0;
    /* ---------------------------------------------- */
    if (conf_in == NULL) return;

    if (conf_in[0] <= 0xfff) {
        if (ConfigCoorMulti(conf_in)) len = 512;
        else if (ConfigFilter(conf_in)) len = 256;
        else if (ConfigKeyMap(conf_in)) len = 241;
        else len = 215;
    } else if (conf_in[1] <= CONFIG_LENGTH) len = conf_in[1];
    else len = CONFIG_LENGTH;
    for (i = 0; i < len; i++) conf[i] = conf_in[i];
    for (; i < CONFIG_LENGTH; i++) conf[i] = 0;
    if (conf_in[0] <= 0xfff) {
        ...
        for (i = 0; i < 4; i++) median_dis[i] = (unsigned char)conf[19 + i];
        for (i = 0; i < 2; i++) {
            match_y[i] = conf[23 + i];
            match_x[i] = conf[25 + i];
        ...
        for (i = 0; i < 4; i++) edge_cut[i] = conf[159 + i];
        for (i = 0; i < 32; i++) stretch_array[i] = conf[163 + i];
        for (i = 0; i < 16; i++) shake_all_array[i] = conf[195 + i];
        reset_mask_dis = conf[213];
        reset_mask_type = conf[214];
        edge_start = conf[216];
        key_map_able = conf[217];
        for (i = 0; i < 8 * 3; i++) key_range_array[i] = conf[218 + i];
        filter_able = conf[242];
        for (i = 0; i < 4; i++) filter_coe[i] = conf[243 + i];
        for (i = 0; i < 4; i++) multi_x_array[i] = conf[247 + i];
        for (i = 0; i < 4; i++) multi_y_array[i] = conf[251 + i];
        diagonal = conf[255];
        for (j = 0; j < 4; j++)
            for (i = 0; i < 64; i++) multi_group[j][i] = conf[256 + i + j * 64];
        for (j = 0; j < 4; j++) {
            for (i = 0; i < 8; i++) {
        ...
        for (i = 0; i < 8 * 3; i++) key_range_array[i] = conf[0x61 + i];
        ...
        for (j = 0; j < 4; j++)
            for (i = 0; i < 64; i++)
                multi_group[j][i] =
                    (conf[0x109 + 64 / 4 * 2 + (i + j * 64) / 4] >> ((i + j * 64) % 4 * 8)) & 0xff;

        filter_able = conf[0x180];
        for (i = 0; i < 4; i++) filter_coe[i] = conf[0x181 + i];
        for (i = 0; i < 4; i++) median_dis[i] = (unsigned char)conf[0x185 + i];
        for (j = 0; j < 4; j++) {
            for (i = 0; i < 8; i++) {
    ...
    for (i = 0; i < 8; i++) {
        if (shake_all_array[i * 2] & 0x8000) shake_all_array[i * 2] = shake_all_array[i * 2] & ~0x8000;
        else shake_all_array[i * 2] = Sqrt(shake_all_array[i * 2]);
    }
    for (i = 0; i < 2; i++) {
        if (match_x[i] & 0x8000) match_x[i] |= 0xffff0000;
        if (match_y[i] & 0x8000) match_y[i] |= 0xffff0000;
        if (ignore_x[i] & 0x8000) ignore_x[i] |= 0xffff0000;
        if (ignore_y[i] & 0x8000) ignore_y[i] |= 0xffff0000;
    }
    for (i = 0; i < CONFIG_LENGTH; i++) config_static[i] = 0;
}
...
static void GetFlag(void) {
    ...
    for (i = AVG_DEEP - 1; i; i--) avg[i] = avg[i - 1];
    avg[0] = 0;
    if ((point_num & 0x8000) != 0) {

        if ((point_num & 0xff000000) == 0x59000000) avg[0] = (point_num >> 16) & 0xff;
    }
    if (((point_num & 0x100) != 0) || ((point_num & 0x200) != 0 && global_state.other.reset == 1)) {
        gsl_id_reg_init(0);
    }
    if ((point_num & 0x300) == 0) global_state.other.reset = 1;

    if (point_num & 0x400) global_state.other.only = 1;
    else global_state.other.only = 0;
    if (point_num & 0x2000) global_state.other.interpolation = INTE_INIT;
    else if (global_state.other.interpolation) global_state.other.interpolation--;
    if (point_num & 0x4000) global_state.other.ex = 1;
    else global_state.other.ex = 0;
    if ((point_num & 0xff) != 0) {
        global_state.other.active_prev = global_state.other.active;
        if ((point_num & 0x800) != 0) global_state.other.active = 1;
        else global_state.other.active = 0;
        if (global_state.other.active != global_state.other.active_prev) {
            if (global_state.other.active) {
                if (prec_id.other.num) gsl_id_reg_init(1);
                else gsl_id_reg_init(0);
    ...
    if (num_save > POINT_MAX) num_save = POINT_MAX;
    for (i = 0; i < POINT_MAX; i++) {
        if (i >= num_save) point_now[i].all = 0;
    }
...
static void PointIgnore(void) {
    ...
    if (id_flag.other.ignore_pri == 0) return;
    for (i = 0; i < point_num; i++) {
        if (point_now[i].other.key) continue;
        y = point_now[i].other.y * (int)screen_y_max / ((int)sen_num_nokey * 64);
        x = point_now[i].other.x * (int)screen_x_max / ((int)drv_num_nokey * 64);
        if ((ignore_y[0] != 0 || ignore_y[1] != 0)) {
            if (y < ignore_y[0]) point_now[i].all = 0;
            if (ignore_y[1] <= screen_y_max / 2 && y > screen_y_max - ignore_y[1]) point_now[i].all = 0;
            if (ignore_y[1] >= screen_y_max / 2 && y > ignore_y[1]) point_now[i].all = 0;
        }
        if (ignore_x[0] != 0 || ignore_x[1] != 0) {
            if (x < ignore_x[0]) point_now[i].all = 0;
            if (ignore_x[1] <= screen_x_max / 2 && x > screen_x_max - ignore_x[1]) point_now[i].all = 0;
            if (ignore_x[1] >= screen_x_max / 2 && x > ignore_x[1]) point_now[i].all = 0;
        }
    }
    x = 0;
    for (i = 0; i < point_num; i++) {
